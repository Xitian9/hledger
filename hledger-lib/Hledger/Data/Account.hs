...
accountsFromPostings ps =
  let
    ...
    summed =  [(aname, foldMap' id amts) | (aname, amts) <- grouped]  -- always non-empty
...
sumAccounts a
  ...
  where
    ...
    ibal = mconcat $ aebalance a : map aibalance subs
...
clipAccountsAndAggregate (Just d) as = combined
    where
      ...
      combined = [a{aebalance=foldMap aebalance same}
