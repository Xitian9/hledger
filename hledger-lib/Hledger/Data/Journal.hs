...
getRunningBalanceB acc = withRunningBalance $ \BalancingState{bsBalances} -> do
  fromMaybe zeromixedamt <$> H.lookup bsBalances acc
...
addToRunningBalanceB acc amt = withRunningBalance $ \BalancingState{bsBalances} -> do
  old <- fromMaybe zeromixedamt <$> H.lookup bsBalances acc
  let new = old <> amt
...
setRunningBalanceB acc amt = withRunningBalance $ \BalancingState{bsBalances} -> do
  ...
  return $ amt <> negateMixedAmount old
...
setInclusiveRunningBalanceB acc newibal = withRunningBalance $ \BalancingState{bsBalances} -> do
  oldebal  <- fromMaybe zeromixedamt <$> H.lookup bsBalances acc
  allebals <- H.toList bsBalances
  let subsibal =  -- sum of any subaccounts' running balances
        foldMap snd $ filter ((acc `isAccountNamePrefixOf`).fst) allebals
  let newebal = newibal <> negateMixedAmount subsibal
  H.insert bsBalances acc newebal
  return $ newebal <> negateMixedAmount oldebal
  ...
  actualbal' <-
    ...
    then
      ...
      withRunningBalance $ \BalancingState{bsBalances} ->
        H.foldM
          (\ibal (acc, amt) -> return $
            if assertedacct==acc || assertedacct `isAccountNamePrefixOf` acc then ibal <> amt else ibal)
          zeromixedamt
          bsBalances
