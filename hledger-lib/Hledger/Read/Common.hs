    ...
    checkcommodities Posting{..} =
      ...
      where
        mfirstundeclaredcomm =
          ...
          (maybe id ((:) . baamount) pbalanceassertion) (filter (/= missingamt) pamount)
...
mamountp' = mixedFromAmount . amountp'
...
tests_Common = tests "Common" [
  ...
  ,tests "spaceandamountormissingp" [
     test "space and amount" $ assertParseEq spaceandamountormissingp " $47.18" (mixedFromAmount $ usd 47.18)
