...
getAmount rules record currency p1IsVirtual n =
  ...
  let
    ...
    assignments = [(f,a') | f <- fieldnames
                          ...
                          , let a' = if f `elem` unnumberedfieldnames && n==2 then mixedAmountCost (negateMixedAmount a) else a
                          ]
  ...
  in case -- dbg0 ("amounts for posting "++show n)
          assignments'' of
      [] -> Nothing
      [(f,a)] | "-out" `T.isSuffixOf` f -> Just (negateMixedAmount a)  -- for -out fields, flip the sign
      [(_,a)] -> Just a
...
parseAmount rules record currency s =
    either mkerror mixedFromAmount $  -- PARTIAL:
