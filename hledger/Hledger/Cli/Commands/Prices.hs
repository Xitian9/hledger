...
prices opts j = do
  let
    ...
    icprices   = map (stylePriceDirectiveExceptPrecision styles) $ concatMap postingsPriceDirectivesFromCosts $ map (postingTransformAmount invertPrice) ps
...
postingsPriceDirectivesFromCosts p = mapMaybe (amountPriceDirectiveFromCost date) $ pamount p
